"""Refresh defining HCPCS and CPTs that are considered to be PT, OT or SLP services"""
...
with DAG(
        dag_id='cms_pt_ot_slp_data_processing',
        description=__doc__,
        ...
        params={
            'local_folder_name': Variable.get(key='local_folder_name', default_var='cms_pt_ot_slp'),
            'root_url': Variable.get(
                key='root_url',
                default_var='https://www.cms.gov/Medicare/Billing/TherapyServices'
            ),
        },
) as dag:
    # region doc
