"""Refresh descriptions, RVU values and payment logic indicators for all CPTs and HCPCS"""
...
with DAG(
        dag_id='pfs_rvu_etl_data_processing',
        description=__doc__,
        ...
        params={
            'local_folder_name': Variable.get(key='local_folder_name', default_var='pfs_rvu'),
            'root_url': Variable.get(
                key='root_url',
                default_var='https://www.cms.gov/Medicare/Medicare-Fee-for-Service-Payment/PhysicianFeeSched/PFS-Relative-Value-Files'
            )
        },
) as dag:
    # region doc
    ...
    main__get_all_pfs_rvu__task = PythonOperator(
        task_id='main__get_all_pfs_rvu__task',
        python_callable=get_all_pfs_rvu,
        doc_md=get_all_pfs_rvu.__doc__,
    )

    main__load_pfs_rvu_to_db__task = PythonOperator(
        task_id='main__load_pfs_rvu_to_db__task',
        python_callable=load_pfs_rvu_to_db,
        doc_md=load_pfs_rvu_to_db.__doc__,
    )
    # endregion
    ...
    ] >> main__get_all_download_urls__task >> main__download_files__task >> main__get_all_pfs_rvu__task >> \
    main__load_pfs_rvu_to_db__task >> teardown__cleanup__task >> [
